...
def make_sequence_vector(sequence, numbers, num_bins, revcomp, revcomp_dictionary, normalize_method, k_values, mismatch, alphabet, kmer_list, boundaries, pseudocount):
    ...
    for k in k_values:
        ...
        for i_seq in range(seq_length):
            ...
            # If we're doing reverse complement, adjust the kmer accordingly.
            if revcomp:
                rev_kmer = find_revcomp(kmer, revcomp_dictionary)
                if kmer > rev_kmer:
                    kmer = rev_kmer
            ...
            # Handle mismatches if required.
            if mismatch != 0:
                for i_kmer in range(k):
                    for letter in alphabet:
                        if kmer[i_kmer] != letter:
                            neighbor = substitute(i_kmer, letter, kmer)
                            # Adjust for reverse complement as needed.
                            if revcomp:
                                rev_kmer = find_revcomp(neighbor, revcomp_dictionary)
                                if neighbor > rev_kmer:
                                    neighbor = rev_kmer
    ...
    # Normalize the vector as required.
    return_value = normalize_vector(normalize_method, k_values, sequence_vector, kmer_list)
    return return_value
...
def read_fasta_sequence(numeric, fasta_file):
    # Leer el primer carácter para verificar si es el inicio de una nueva secuencia
    first_char = fasta_file.read(1)
    if first_char == "":
        # No hay más datos en el archivo
        return ["", ""]
    elif first_char != ">":
        # Esto no debería ocurrir si el archivo está bien formado
        sys.stderr.write("Expected '>' but got: {}\n".format(first_char))
        sys.exit(1)

    # Leer el resto de la línea de encabezado
    header = fasta_file.readline().strip()

    # Aquí simplemente podrías tomar todo el encabezado como ID, o adaptar según tu archivo específico
    id = header  # Toma toda la línea de encabezado como ID
    # Si necesitas procesar el encabezado puedes hacerlo aquí, por ejemplo:
    # if "=" in header:
    #     id = header.split('=')[1]  # Asumiendo que quieres lo que sigue al '='
    # else:
    #     id = header

    # Leer la secuencia hasta el próximo '>'
    sequence = []
    while True:
        line = fasta_file.readline()
    ...
    # Concatenar todas las partes de la secuencia y eliminar nuevas líneas o espacios si necesario
    clean_sequence = ''.join(sequence).replace('\n', '').replace(' ', '').upper() if not numeric else ''.join(sequence).replace('\n', '').replace(' ', '')

    return [id, clean_sequence]

# Update 2
def read_fasta_sequence(numeric, fasta_file):
    # Leer la línea de encabezado inicial
    header = fasta_file.readline().strip()
    if not header.startswith('>'):
        raise ValueError("Expected '>' at the beginning of the header, but got: {}".format(header[0]))
    
    # Extraer la identificación del encabezado
    id = header[1:].split()[0]  # Esto toma la primera palabra del encabezado, excluyendo el '>'

    # Concatenar las líneas de la secuencia hasta el próximo encabezado
    sequence = []
    while True:
        pos = fasta_file.tell()  # Guarda la posición actual del archivo
        line = fasta_file.readline()
        if not line or line.startswith('>'):
            fasta_file.seek(pos)  # Regresa al inicio de esta línea si es un nuevo encabezado
            break
        sequence.append(line.strip())

    # Unir todas las partes de la secuencia en una cadena, eliminando espacios y nuevas líneas
    clean_sequence = ''.join(sequence).replace('\n', '').replace(' ', '').upper() if not numeric else ''.join(sequence).replace('\n', '').replace(' ', '')
...
# python preprocessing\fasta2matrix.py 4 data\Milseq.fasta output.csv
